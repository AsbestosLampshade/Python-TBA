...
import time
win= GraphWin('Game',800,700)
class player:
    global x,y
    x=250;y=250
    Plpt= Point(x,y)
    def opener(self,pt):
        global playcir,playcir1,playcir2
        playcir= Circle(pt,10)
        playcir.draw(win)
        playcir.setFill('green')
        playcir1=Circle(Point(x+5,y),3)
        playcir2=Circle(Point(x-5,y),3)
        playcir1.draw(win)
        playcir2.draw(win)
        playcir2.setFill('black')
        playcir1.setFill('black')
        #time.sleep(3)
    def N(self):
        global y
        y=y-50
...
def player_movement(player_input):
    ...
    #if coordinates in map_walls==False:
    if player_input=="UP":
